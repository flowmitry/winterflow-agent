...
func NewClient(config *config.Config, appRepository repository.AppRepository, registryRepository repository.DockerRegistryRepository) (*Client, error) {
	...
	if err := command.RegisterCommandHandlers(commandBus, config, appRepository, registryRepository); err != nil {
		cancel()
	...
	if err := query.RegisterQueryHandlers(queryBus, config, appRepository, registryRepository); err != nil {
		cancel()
